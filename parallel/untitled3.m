...
dh = 1/6*dp.*ep./(1-ep);
VEL = 0;
TT= nonLinspace( 500, Tout ,numel(X)-1, 'exp10' );
%TT = -47.187.*xx.^2 + 2.7339e+05.*xx + 231.6529;

for i = 1:numel(X)-1
    prantl(i) = (visc(TT(i)).*H_cp(TT(i)).*1e3)./conduc(TT(i));
    
    Area = 2.*pi.*(ROUT-xx(i)) .*hig; % im taking the area of half of the slice
    AA(i) = Area; 
